...
const { createRoom, addParticipantToRoom } = require("../services/acsRoomsService"); // ADD THIS LINE
...
try {
  ...
  scheduledDateTime = new Date(Date.now() + 60 * 60 * 1000);
    ...
    const validUntil = new Date(scheduledDateTime.getTime() + 8 * 60 * 60 * 1000); // 8 hours later

    console.log("Scheduled time:", scheduledDateTime.toISOString());
    console.log("Valid until:", validUntil.toISOString());

    const room = await createRoom(new Date(), new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)); // Valid NOW for 30 days
    console.log("ACS Room created successfully:", room.id);
    
    const threadId = `trial-case-${caseId}-${Date.now()}`;
    const meetingId = await TrialMeeting.createMeeting(caseId, threadId, room.id);

    console.log(`Trial meeting stored in DB, Meeting ID: ${meetingId}, Room ID: ${room.id}`);
  ...
  try {
    ...
    const trials = await db.query(`
      ...
      WHERE c.Status = 'approved'
        AND c.ScheduledDate >= ?
        AND c.ScheduledDate < ?
      ORDER BY c.ScheduledDate, c.ScheduledTime
    `, [today, tomorrow]);

    res.json({ success: true, trials: trials[0] });
  } catch (error) {
...
router.post('/admin-join/:caseId', async (req, res) => {
  try {
    ...
    const [cases] = await db.query(
      'SELECT RoomId, ScheduledDate, ScheduledTime FROM Cases WHERE CaseId = ? AND Status = "approved"',
      [caseId]
    );

    if (cases.length === 0) {
      return res.status(404).json({ success: false, message: 'Trial not found or not approved' });
    }

    const trial = cases[0];

    // Generate ACS token for admin
    const acsToken = await generateACSToken(); // Your existing ACS token generation
    ...
    res.status(500).json({ success: false, message: 'Failed to join trial' });
  }
});
