...
router.get("/cases/available", requireJurorOnboarding, async (req, res) => {
  try {
    ...
    console.log("Juror ID:", juror.id);
    console.log("State:", juror.state);
    console.log("County:", juror.county);

    const Case = require("../models/Case");

    // FIXED: Pass both county AND jurorId to filter out already applied cases
    const availableCases = await Case.getAvailableCasesForJurors(
      juror.county,
      juror.id // Add jurorId parameter
    );
    ...
    if (availableCases.length > 0) {
      console.log("=== FIRST CASE DEBUG ===");
      console.log(
        "First case raw data:",
        JSON.stringify(availableCases[0], null, 2)
      );
      console.log("RequiredJurors value:", availableCases[0].RequiredJurors);
      console.log(
        "RequiredJurors type:",
        typeof availableCases[0].RequiredJurors
      );
      console.log("ApprovedJurors value:", availableCases[0].ApprovedJurors);
      console.log(
        "ApprovedJurors type:",
        typeof availableCases[0].ApprovedJurors
      );
      console.log(
        "Calculation:",
        availableCases[0].RequiredJurors - availableCases[0].ApprovedJurors
      );
    }
...
// In jurorRoutes.js - Get my applications
router.get("/applications", async (req, res) => {
  try {
    const jurorId = req.user.id;
    const JurorApplication = require("../models/JurorApplication");

    // This should include AttorneyStatus from Cases table
    const applications = await JurorApplication.getApplicationsByJuror(jurorId);

    res.json({
      success: true,
      applications, // Should include AttorneyStatus field
    });
  } catch (error) {
    console.error("Get my applications error:", error);
    res.status(500).json({
