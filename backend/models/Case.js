 ...
 * Excludes cases the juror has already applied to
 */
async function getAvailableCasesForJurors(county = null, jurorId = null) {
  try {
    const pool = await poolPromise;
    let query = `
      SELECT 
        ...
        a.FirstName + ' ' + a.LastName AS AttorneyName,
        a.LawFirmName,
        CAST(ISNULL(c.RequiredJurors, 6) AS INT) AS RequiredJurors,
        (
          SELECT COUNT(*) 
          FROM dbo.JurorApplications ja 
          WHERE ja.CaseId = c.CaseId 
    ...
    // Exclude cases this juror has already applied to
    if (jurorId) {
      query += ` AND c.CaseId NOT IN (
        SELECT CaseId 
    ...
    // Only show cases that still need jurors
    query += ` 
      AND (
        SELECT COUNT(*) 
        FROM dbo.JurorApplications ja 
