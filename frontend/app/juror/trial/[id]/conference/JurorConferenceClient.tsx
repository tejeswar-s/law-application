...
export default function JurorConferenceClient() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (localRenderer.current) {
        localRenderer.current.dispose();
      }
    ...
    return (
      <div className="h-screen bg-gray-900 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 max-w-md">
          ...
          <button onClick={() => router.push("/juror")} className="w-full py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
            Back to Dashboard
          </button>
        </div>
      </div>
    ...
    <div className="h-screen bg-gray-900 flex flex-col">
      <div className="bg-gray-800 px-6 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white font-semibold">Quick Verdicts - Virtual Courtroom</span>
        </div>
        <div className="text-white text-sm">
          Status: <span className={callState === "Connected" ? "text-green-400" : "text-yellow-400"}>{callState}</span>
        </div>
      </div>
      ...
      <div className="flex-1 p-6 flex flex-col overflow-hidden">
            ...
            {featuredVideo ? (
              ...
              <>
                {featuredParticipant === "local" ? (
                  <>
                    <div ref={featuredVideoRef} className="w-full h-full [&>div]:!w-full [&>div]:!h-full [&_video]:object-contain" />
                    {isVideoOff && (
                      <div className="absolute inset-0 flex items-center justify-center bg-gray-800">
                        <div className="w-32 h-32 bg-gray-600 rounded-full flex items-center justify-center">
                          <span className="text-white text-4xl font-bold">You</span>
                        </div>
                      </div>
                    )}
                  </>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 flex-shrink-0">
          {featuredParticipant !== "local" && (
            <div 
              className="flex-shrink-0 w-48 h-32 bg-black rounded-lg relative overflow-hidden cursor-pointer border-2 border-transparent hover:border-gray-500"
      ...
      <div className="bg-gray-800 px-6 py-4 flex items-center justify-center gap-4 flex-shrink-0">
        <button 
          onClick={toggleMute} 
          className={`px-6 py-3 rounded-lg ${isMuted ? "bg-red-600" : "bg-gray-700"} text-white hover:opacity-80`}
        >
          {isMuted ? "Unmute" : "Mute"}
        </button>
        <button 
          onClick={toggleVideo} 
          className={`px-6 py-3 rounded-lg ${isVideoOff ? "bg-red-600" : "bg-gray-700"} text-white hover:opacity-80`}
        >
