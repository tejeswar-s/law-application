...
export default function JurorQuizOverlay({ open, onClose, onFinish, sidebarCollapsed = false }: { open: boolean; onClose: () => void; onFinish: () => void; sidebarCollapsed?: boolean; }) {
  ...
  return (
    <div
      className="fixed inset-y-0 z-50 bg-black/40 flex items-stretch justify-stretch transition-all duration-500 ease-in-out"
      style={{ 
        left: sidebarWidth, 
        ...
        <div className="flex-1 flex flex-col px-12 pt-12 pb-8">
          {step === "quiz" && (
            <>
              <div className="w-full">
                ...
                <div className="flex flex-col gap-4 mb-8">
                  {q.options.map(opt => (
                    <button
                      key={opt}
                      className={`flex items-center gap-2 px-5 py-2 border rounded-md text-[16px] font-medium transition-colors text-left cursor-pointer ${selected === opt ? 'border-[#0C2D57] bg-[#F3F6FA]' : 'border-gray-300 bg-white hover:border-[#0C2D57]'}`}
              ...
              <div className="flex justify-end w-full">
                <button
                  className={`px-6 py-2 rounded font-semibold text-white text-lg transition-colors cursor-pointer ${selected ? 'bg-[#0C2D57] hover:bg-[#0a2342]' : 'bg-gray-300 cursor-not-allowed'}`}
          ...
          {step === "correct" && (
            <div className="flex flex-col items-center justify-center w-full h-full">
              ...
              <button
                className="px-6 py-2 rounded font-semibold text-white text-lg bg-[#0C2D57] hover:bg-[#0a2342] cursor-pointer"
                onClick={() => {
